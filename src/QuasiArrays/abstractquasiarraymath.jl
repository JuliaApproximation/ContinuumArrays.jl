...
"""
    vec(a::AbstractQuasiArray) -> AbstractQuasiVector

Reshape the array `a` as a one-dimensional column vector. Return `a` if it is
already an `AbstractQuasiVector`. The resulting array
...
"""
vec(a::AbstractQuasiArray) = reshape(a,length(a))
vec(a::AbstractQuasiVector) = a
...
```
"""
function reverse(A::AbstractQuasiArray; dims::Integer)
    nd = ndims(A); d = dims
...
function circshift(a::AbstractQuasiArray, shiftamt::Real)
    circshift!(similar(a), a, (Integer(shiftamt),))
end
circshift(a::AbstractQuasiArray, shiftamt::DimsInteger) = circshift!(similar(a), a, shiftamt)
"""
    circshift(A, shifts)
...
"""
function circshift(a::AbstractQuasiArray, shiftamt)
...
"""
    repeat(A::AbstractQuasiArray, counts::Integer...)
...
```
"""
repeat(a::AbstractQuasiArray, counts::Integer...) = repeat(a, outer = counts)

function repeat(a::AbstractQuasiVecOrMat, m::Integer, n::Integer=1)
...
function repeat(a::AbstractQuasiVector, m::Integer)
    o = length(a)
    b = similar(a, o*m)
    for i=1:m
        c = (i-1)*o+1
...
"""
    repeat(A::AbstractQuasiArray; inner=ntuple(x->1, ndims(A)), outer=ntuple(x->1, ndims(A)))
...
```
"""
function repeat(A::AbstractQuasiArray; inner = nothing, outer = nothing)
    return _repeat_inner_outer(A, inner, outer)
end

# we have optimized implementations of these cases above
_repeat_inner_outer(A::AbstractQuasiVecOrMat, ::Nothing, r::Union{Tuple{Integer},Tuple{Integer,Integer}}) = repeat(A, r...)
_repeat_inner_outer(A::AbstractQuasiVecOrMat, ::Nothing, r::Integer) = repeat(A, r)
...
rep_kw2tup(v::AbstractQuasiArray{<:Integer}) = (v...,)
...
@noinline function _repeat(A::AbstractQuasiArray, inner, outer)
